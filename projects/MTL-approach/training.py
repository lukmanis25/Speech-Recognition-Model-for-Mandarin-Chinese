...
class Main:
    def __init__(self, model_path: pathlib.Path, n_inputs: int, n_hidden: int, n_layers: int, lr: float, weight_decay: float, batch_size: int):
        ...
        self.model = PinyinTransformer(N_TOKENS, n_inputs, n_head, n_hidden, n_layers).to(self.device)
        ...
        self.transform_mfcc = torchaudio.transforms.MFCC(sample_rate=new_sample_rate, n_mfcc=13) # (sample_rate=16000, n_mfcc=13, melkwargs={"n_fft": 400, "hop_length": 80, "n_mels": 23, "center": False}) # n_mfcc=13 (default:40), , log_mels=True

        self.transform_resample = self.transform_resample.to(self.device)
        self.transform_mfcc = self.transform_mfcc.to(self.device)
    ...
    def train(self, epoch: int, log_interval: int, pbar: tqdm, pbar_update: float):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            """
            TODO: poniższe i zapisywanie bazy danych (najlepiej w moim formacie pośrednim)
            File "C:\Users\kubak\MGR_SEM1\PB\NASZE_REPO\projects\MTL-approach\training.py", line 231, in train
                data = transform(data).to(self.device)
                    ^^^^^^^^^^^^^^^
            File "C:\Users\kubak\MGR_SEM1\PB\NASZE_REPO\projects\MTL-approach\venv\Lib\site-packages\torch\nn\modules\module.py", line 1736, in _wrapped_call_impl
                return self._call_impl(*args, **kwargs)
                    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            File "C:\Users\kubak\MGR_SEM1\PB\NASZE_REPO\projects\MTL-approach\venv\Lib\site-packages\torch\nn\modules\module.py", line 1747, in _call_impl
                return forward_call(*args, **kwargs)
                    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            File "C:\Users\kubak\MGR_SEM1\PB\NASZE_REPO\projects\MTL-approach\venv\Lib\site-packages\torchaudio\transforms\_transforms.py", line 979, in forward
                return _apply_sinc_resample_kernel(waveform, self.orig_freq, self.new_freq, self.gcd, self.kernel, self.width)
                    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            File "C:\Users\kubak\MGR_SEM1\PB\NASZE_REPO\projects\MTL-approach\venv\Lib\site-packages\torchaudio\functional\functional.py", line 1466, in _apply_sinc_resample_kernel
            ...
            print(f"print data size and abort ;p\n{data.size()}")
    ...
    def test(self, epoch: int, pbar: tqdm, pbar_update):
        # invalid_num = 0
        # good_guess = 0
    ...
    def run(self):
        ...
        with tqdm(total=n_epoch) as pbar:
            for epoch_iter in range(self.epoch + 1, self.epoch + n_epoch + 1):
                ...
                self.test(epoch_iter, pbar, pbar_update)
